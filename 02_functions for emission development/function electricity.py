...
# Data from projections of the German Federal Environmental Agency (2024) for the share of non-renewable electricity from 2025 to 2045
# (https://www.umweltbundesamt.de/dokument/datenanhangkernindikatoren-projektionsbericht-2024)
years = np.array([
...
values = np.array([
    42.2, 38.1, 35.3, 33.2, 30.1, 25.8, 21.6, 18.1, 14.9,
    12.2, 10.4, 9.0, 7.6, 6.3, 5.1, 4.0, 4.0, 4.1, 4.2,
    4.3, 4.3
])
...
for i, (func, name, x, p0) in enumerate(functions):
    try:
        ...
        plt.plot(years, y_pred, label=f"{name} (R²={r2:.4f})", color=color_cycle[i % len(color_cycle)], linewidth=2)
...
plt.ylabel("Share of non-renewable \ngross electricity consumption [%]", fontsize=16)
